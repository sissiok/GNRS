...
######################################################################################
# modified by Feixiong Zhang to adapt to the click delay config file format requirement for server.
...
GEN_OUTPUT: {
  foreach $asNum (@asList) {
    ...
    open($outFile, ">", "as_".$Count."_delay_serv.dat");
	...
	$count++;
        ...
        if($srcAS == $asNum) {
	  my $dstIP = $count + 100;
          print $outFile "192.168.1.$dstIP, $clientPort, $intraDelay,\n";
          next;
        }
