...
bool CSHA1::ReportHash(TCHAR* tszReport, REPORT_TYPE rtReportType) const
{
	...
	if((rtReportType == REPORT_HEX) || (rtReportType == REPORT_HEX_SHORT)) //report hex 
	{
		_sntprintf(tszTemp, 15, _T("%02X"), m_digest[0]); //Store 15 characters into buffer (first element)
		_tcscpy(tszReport, tszTemp);  //copy buffer to report

		const TCHAR* lpFmt = ((rtReportType == REPORT_HEX) ? _T(" %02X") : _T("%02X"));
		for(size_t i = 1; i < 20; ++i)		//iterate for 19 times 
		{
...
u64b CSHA1::curr_GUID =0; 
u32b CSHA1::hash_result[] ={0}; 

//Init by GUID
bool CSHA1::reInit(u64b _GUID){

	if (curr_GUID != _GUID){
		Reset(); 
		CSHA1::curr_GUID = _GUID;
		return true; 
	}
	return false; 
}

//put the result into hash_result vector
void CSHA1::Havest(){
	u32b tmpt; 
	u8b j=0; 
	...
	for (u8b i=0; i<5; i++){
		...
		for (int exp=3; exp >=0; exp--){
			tmpt+= m_digest[j]*(u32b)pow(256,(float)exp); 
			j++; 
		}		
		hash_result[i] = tmpt; 
	}
