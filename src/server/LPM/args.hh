...
class Args : public ArgContext {
  ...
  public:
    ...
    static const int mandatory = 1;  ///< read flag for mandatory arguments
    static const int positional = 2; ///< read flag for positional arguments
    static const int deprecated = 4; ///< read flag for deprecated arguments
    static const int firstmatch = 8; ///< read flag to take first matching argument
...
class IntArg : public NumArg { public:
	...
	(void) args;
	const bool is_signed = integer_traits<V>::is_signed;
	const int nlimb = int((sizeof(V) + sizeof(limb_type) - 1) / sizeof(limb_type));
	limb_type x[nlimb];
