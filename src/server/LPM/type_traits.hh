...
template <typename T> struct fast_argument {
    static const bool is_reference = !has_trivial_copy<T>::value || sizeof(T) > sizeof(void *);
    typedef typename conditional<is_reference, const T &, T>::type type;
};
template <typename T> const bool fast_argument<T>::is_reference;
...
template <typename T> const bool integer_traits<T>::is_numeric;
template <typename T> const bool integer_traits<T>::is_integral;
...
struct make_fast_half_integer {
    ...
    static const int half_bits = int(sizeof(type) * 4);
    static const type half_value = type(1) << half_bits;
