...
extern ofstream ProcFile;
extern int rec_insert_num,rec_lookup_num,proc_insert_num,proc_lookup_num;
...
void* statisticsProc(void* arg)
{
  ...
  while(1)  {
	...
	ProcFile<<i<<" "<<rec_insert_num<<" "<<proc_insert_num<<" "<<rec_lookup_num<<" "<<proc_lookup_num<<endl;
	i++;
  }
  return NULL;
}
