...
uint32_t Common::port_str2num(char *listen_port)  {
        int length=strlen(listen_port);
        uint32_t port_num=0;
	if(listen_port[0]=='-') 
		port_num=-1;
	else {
	        for(int i=0;i<length;i++)
        	        port_num=port_num*10+(int)listen_port[i]-48;
	}
        return(port_num);
}
...
/*
	...
	Input: unsigned int version of IP address
	Output: Vector of 4 elements of an IP 
*/
vector<u8b> Common::num2ip(u32b _ip){

	vector<u8b> _result; 
	vector<u8b>::iterator _vItt;
	u32b temp=4278190080U;
	u32b ip_temp;
	for (int i=0;i<=3;i++){
		ip_temp=_ip&temp;
		ip_temp=ip_temp>>24;
		_result.push_back((u8b)ip_temp);
		_ip=_ip<<8;
	}
